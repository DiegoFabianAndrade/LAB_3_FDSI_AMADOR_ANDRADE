...
|---|---|---|---|---|---|
| **RSK-01** | Fuga de versión de software (`Server: nginx/1.18.0`) | Information Disclosure (H2) | **Corregido** | Se configuró `server_tokens off;` en el bloque de servidor Nginx. El retest confirma que solo responde `Server: nginx`. | Control mantenido y reforzado. |
...
| **RSK-03** | Exposición de archivos de sistema ocultos (`.git/config`) | Information Disclosure (H2) | **Corregido** | Se añadió la directiva `location ~ /\. { deny all; }` denegando el acceso con un código HTTP 403. | Control mantenido. |
...
| **RSK-05** | Transmisión de peticiones y respuestas en texto plano (HTTP sin TLS) | Information Disclosure / Tampering (H1/H4) | **Aceptado (Límite Pedagógico)** | Riesgo documentado explícitamente y aceptado temporalmente para el alcance del Laboratorio 3. | **Pendiente para Lab 4**: Implementación de TLS/HTTPS y certificados digitales. |
| **RSK-06** | Acceso no autenticado a la API de incidentes CrowdStrike Falcon | Spoofing / Tampering | **Aceptado (Límite Pedagógico)** | Permite probar el prototipo de automatización HTTP sin autenticación en la línea base del Lab 3. | **Pendiente para Lab 4**: Implementación de identidad, tokens JWT/OAuth2 y control de roles. |
